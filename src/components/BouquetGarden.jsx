...
const STYLES = `
  ...
  .bq-orbit-card {
    position:absolute; border-radius:50%; overflow:hidden; z-index:6;
    background:#FFE4EC;
    box-shadow:0 12px 30px rgba(232,160,184,0.3),0 0 0 4px rgba(255,255,255,0.8);
    cursor:pointer;
  }
  ...
  .bq-orbit-label {
    ...
    color:#fff; padding:18px 6px 7px; font-size:0.68rem; font-weight:700;
  ...
  .bq-nav-btn {
    ...
    border:2.5px solid rgba(196,69,105,0.3);
    background:rgba(255,255,255,0.92);
    backdrop-filter:blur(12px);
    display:flex; align-items:center; justify-content:center;
    cursor:pointer; z-index:30; outline:none; user-select:none;
    transition:background 0.3s,border-color 0.3s,box-shadow 0.3s,
               transform 0.35s cubic-bezier(0.34,1.56,0.64,1);
    box-shadow:0 6px 24px rgba(196,69,105,0.18),0 2px 8px rgba(0,0,0,0.06),
               inset 0 1px 0 rgba(255,255,255,0.9);
    overflow:hidden;
  }
  .bq-nav-btn:hover {
    background:linear-gradient(135deg,#FFE4EC,#fff) !important;
    border-color:rgba(196,69,105,0.6) !important;
    box-shadow:0 10px 32px rgba(196,69,105,0.3) !important;
  }
  .bq-nav-btn:active { transform:translateY(-50%) scale(0.92) !important; }
  .bq-nav-btn::before {
    content:''; position:absolute; inset:-4px; border-radius:50%;
    border:2px solid rgba(196,69,105,0.4); opacity:0; transform:scale(1);
    pointer-events:none;
  }
  ...
  /* ── CENTER CARD INNER — responsive font/layout ── */
  .bq-card-category {
    font-size: 0.65rem; color: #B89980;
    text-transform: uppercase; letter-spacing: 0.18em;
    margin-bottom: 4px; font-weight: 700;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .bq-card-title {
    font-size: 1.3rem; font-weight: 800; color: #7A5C4E;
  ...
  .bq-card-rating {
    ...
    margin-bottom: 12px; color: #FBD47A; font-size: 0.9rem;
    flex-wrap: nowrap;
  }
  .bq-card-rating .bq-count {
    color: #9E7B6C; font-size: 0.78rem; margin-left: 4px;
    white-space: nowrap;
  }
  .bq-card-price-row {
  ...
  .bq-price-current {
    font-size: 1.55rem; font-weight: 800; color: #C44569;
    white-space: nowrap;
  }
  .bq-price-old {
    font-size: 0.95rem; color: #B89980;
  ...
  .bq-buy-btn {
    ...
    font-size: 0.82rem; padding: 10px 20px;
  }

  /* ── MOBILE overrides for center card content ── */
  @media (max-width: 600px) {
    .bq-card-info         { padding: 8px 10px !important; }
    .bq-card-category     { font-size: 0.5rem !important; margin-bottom: 2px !important; letter-spacing: 0.1em !important; }
    .bq-card-title        { font-size: 0.78rem !important; margin-bottom: 3px !important; -webkit-line-clamp: 2; }
    .bq-card-rating       { font-size: 0.6rem !important; margin-bottom: 4px !important; gap: 2px !important; }
    .bq-card-rating .bq-count { font-size: 0.55rem !important; }
    .bq-price-current     { font-size: 0.88rem !important; }
    .bq-price-current .bq-currency { font-size: 0.7rem !important; }
    .bq-price-old         { font-size: 0.62rem !important; }
    .bq-buy-btn           { font-size: 0.6rem !important; padding: 5px 9px !important; gap: 3px !important; }
    .bq-buy-btn svg       { width: 9px !important; height: 9px !important; }
    .bq-card-price-row    { gap: 4px !important; }
    .bq-card-price        { gap: 3px !important; }
    /* ribbon + badge smaller */
    .bq-ribbon  { top: 10px !important; padding: 4px 10px 4px 8px !important; font-size: 0.48rem !important; }
    .bq-badge   { top: 8px !important; right: 8px !important; width: 36px !important; height: 36px !important; }
    .bq-badge-pct  { font-size: 0.72rem !important; }
    .bq-badge-off  { font-size: 0.38rem !important; }
  }

  @media (max-width: 380px) {
    .bq-card-info         { padding: 6px 8px !important; }
    .bq-card-title        { font-size: 0.7rem !important; }
    .bq-price-current     { font-size: 0.8rem !important; }
    .bq-buy-btn           { font-size: 0.55rem !important; padding: 4px 7px !important; }
  }
...
function OrbitCircle({ products, currentIdx, posClass, animDirection }) {
  ...
  useEffect(() => {
    ...
    if (curImg)  curImg.className  = 'bq-orbit-img bq-leaving'
    if (nextImg) {
    ...
    const timer = setTimeout(() => {
      if (nextImg) nextImg.className = 'bq-orbit-img bq-active'
      if (curImg)  { curImg.className = 'bq-orbit-img'; curImg.style.opacity = '' }
    }, ANIM_DURATION + 50)
  ...
  const posClassMap = {
    'pos-1':'bq-pos-1','pos-2':'bq-pos-2','pos-3':'bq-pos-3',
    'pos-7':'bq-pos-7','pos-8':'bq-pos-8',
  }

  return (
    <div
      className={`bq-orbit-card ${posClassMap[posClass]}`}
      style={{ width:130, height:130, ...posStyles[posClass] }}
    ...
    >
      ...
      <div className="bq-orbit-label">{products[currentIdx]?.name || ''}</div>
    </div>
  )
}
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div
            ...
            style={{
              ...
              background:'linear-gradient(135deg,#C44569,#E8607B)',
              color:'#fff', padding:'8px 18px 8px 14px',
              fontSize:'0.7rem', fontWeight:800, letterSpacing:'0.15em',
              boxShadow:'0 4px 14px rgba(196,69,105,0.4)', zIndex:3,
          ...
          {discount && (
            <div
              ...
              style={{
                ...
                width:60, height:60,
                background:'linear-gradient(135deg,#FBD47A,#F4B860)',
                borderRadius:'50%', display:'flex', flexDirection:'column',
                alignItems:'center', justifyContent:'center',
                fontWeight:900, boxShadow:'0 6px 18px rgba(244,184,96,0.5)', zIndex:3,
              }}
            >
              <span className="bq-badge-pct" style={{ fontSize:'1.1rem', lineHeight:1, color:'#7A5C4E' }}>{discount}%</span>
              <span className="bq-badge-off" style={{ fontSize:'0.55rem', letterSpacing:'0.1em', color:'#7A5C4E' }}>OFF</span>
            </div>
          )}

          {/* Product image — 57% height on mobile, 62% desktop */}
          <div style={{
            position:'relative', width:'100%', height:'57%',
            overflow:'hidden',
            background:'linear-gradient(135deg,#FFE4EC,#FFF0E5)',
            flexShrink: 0,
          }}>
            <img
              className="bq-center-img"
          ...
          >
            <div style={{ minHeight:0, overflow:'hidden' }}>
              {/* Category */}
              <div className="bq-card-category">
                {p.category?.replace(/-/g,' ') || 'Flowers'}
              </div>
              {/* Name */}
              <h3 className="bq-card-title">{p.name}</h3>
              {/* Rating */}
              <div className="bq-card-rating">
                <span>★★★★</span>
                <span style={{ opacity:0.4 }}>★</span>
                <span className="bq-count">({p.reviewCount || 0} reviews)</span>
              </div>
            </div>

            {/* Price + Buy */}
            <div className="bq-card-price-row">
              <div className="bq-card-price">
                <span className="bq-price-current">
                  <span className="bq-currency" style={{ fontSize:'1rem' }}>₹</span>
                  {p.price}
                </span>
                {p.originalPrice && (
                  <span className="bq-price-old">₹{p.originalPrice}</span>
                )}
              </div>
              <button className="bq-buy-btn" onClick={handleBuy}>
                Buy Now
        ...
        <div style={{
          position:'absolute', inset:0,
          backfaceVisibility:'hidden', WebkitBackfaceVisibility:'hidden',
          transform:'rotateY(180deg)', background:'#FFE4EC',
        }}>
...
export default function BouquetGarden({ products = [] }) {
  ...
  return (
    ...
    >
      ...
      {/* Stage */}
      <div className="bq-stage" style={{ position:'relative', maxWidth:1100, margin:'0 auto', height:800, zIndex:5 }}>

        {/* Wreath */}
        <div className="bq-wreath" style={{ position:'absolute', top:420, left:'50%', transform:'translate(-50%,-50%)', width:640, height:640, border:'2px dashed rgba(232,160,184,0.3)', borderRadius:'50%', pointerEvents:'none', zIndex:2 }}>
        ...
        {/* Left arrow */}
        <button className="bq-nav-btn bq-nav-prev"
        ...
        {/* Right arrow */}
        <button className="bq-nav-btn bq-nav-next"
          onClick={() => navigate_orbit(1)}
