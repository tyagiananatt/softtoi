...
router.get('/:id', async (req, res) => {
  try {
    const product = await Product.findById(req.params.id)
      .select('-image -images')  // exclude heavy base64 fields
      .maxTimeMS(10000)
      .lean();
    if (!product) return res.status(404).json({ message: 'Product not found' });
    
    // Add image URLs (both imageUrl and image for compatibility)
    const base = `${req.protocol}://${req.get('host')}`;
    const imageUrl = `${base}/api/products/${product._id}/image`;
    product.imageUrl = imageUrl;
    product.image = imageUrl;  // for backward compatibility
    product.images = [];  // detail page only needs main image
    ...
    if (product.variantGroup) {
      variants = await Product.find({ variantGroup: product.variantGroup })
        ...
        .lean();
      // Add image URLs for variants
      variants.forEach(v => {
        const vImageUrl = `${base}/api/products/${v._id}/image`;
        v.imageUrl = vImageUrl;
        v.image = vImageUrl;  // for backward compatibility
      });
    }
